...
class DAOFacadeImpl : DAOFacade {
    ...
    override suspend fun addUser(email: String, userType: String, password: String): User? = dbQuery {
        val userDoesNotExist = Users.select { Users.email eq email }.empty()
        if (userDoesNotExist) {
    ...
    // Grab the user that matches the email from the db, if the password match return user else return null
